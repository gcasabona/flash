...
for line in f:
	...
	if (counter%2):
	    ...
	    he4_abundance.append(float (lst [1])) #GC
	    he3_abundance.append(float (lst [2])) #GC
#print(time)
#print(min(t_burn))
#print(max(t_burn))
...
#plt.show()
	...
	#GC: writing data to file
	a = eddy_time/np.asarray(t_burn)
	zip(a,time)
	import csv
	with open("timescale.txt", "w") as f:
	  writer = csv.writer(f, delimiter='\t')
	  writer.writerows(zip(a,time))
	quit()
	...
	color = 'tab:red'
	#axis_2.set_ylabel('c12_abundance', color = color)
	axis_2.set_ylabel(r'min ($X_{4}$ abundance)',fontsize =12, color = color)
	axis_2.plot(time, np.asarray(he4_abundance),color = color)
	axis_2.tick_params(axis='y',labelcolor = color)
	#axis_2.legend(loc = 'center left')
	#fig.savefig('local_timescale_vs_local_c12.png',format = 'png', dpi = 1000)
	...
	color = 'tab:red'
	#axis_2.set_ylabel('c12_abundance', color = color)
	axis_2.set_ylabel(r'min ($X_{3}$ abundance)',fontsize =12, color = color)
	axis_2.plot(time, np.asarray(he3_abundance),color = color)
	axis_2.tick_params(axis='y',labelcolor = color)
	#axis_2.legend(loc = 'center left')
	#fig.savefig('local_timescale_vs_local_c12.png',format = 'png', dpi = 1000)
